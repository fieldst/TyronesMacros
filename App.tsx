...
const App: React.FC = () => {
  ...
  // 🔴 NEW: persist targets globally so they’re remembered across sessions/days
  ...
  useEffect(() => {
    const needsMigration = days.some(d => d.workoutKcal === undefined);
    if (needsMigration) {
      setDays(currentDays =>
        currentDays.map(d => ({
          ...d,
  ...
  // Ensure today exists; if not, create it using the *persisted* targets
  useEffect(() => {
    ...
    if (!todayEntry) {
      setDays(prevDays => [
        ...
        {
          date: todayDate,
          targets: { ...targets }, // 🔴 use saved targets for new day
          workoutLogged: '',
          workoutKcal: 0,
          swapSuggestions: '',
        },
  ...
  useEffect(() => {
    ...
    if (todayEntry && (
      ...
      // optional: sync saved targets to today’s record if they differ
      // Comment out if you prefer "per-day" divergence after manual edits
  ...
  }, [mealPendingDelete, setMeals]);

  // ---------- Workout: update / delete ----------
  const handleUpdateWorkout = (workoutText: string, workoutKcal: number) => {
  ...
  const performDeleteWorkout = useCallback(() => {
    if (!today) {
    ...
    setDays(prev => {
      ...
      const next = prev.map(d => {
        if (d.date === targetDate) {
          changed = true;
          return { ...d, workoutLogged: "", workoutKcal: 0 };
        }
        return d;
      });
  ...
  // 🔴 When targets are updated anywhere, we update:
  // 1) the saved global targets (tm_targets), and
  // 2) today’s targets record.
  const handleUpdateTargets = (newTargets: MacroSet) => {
    setTargets(newTargets); // persists globally
  ...
  const renderView = () => {
    switch (activeView) {
      case 'history': return <HistoryView days={days} meals={meals} />;
      case 'targets': return (
      ...
      default:
        return (
          <TodayView
            ...
            // 🔴 NEW: allow Today view to open the Targets tab quickly
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 flex flex-col font-sans">
      <header className="bg-primary text-white shadow-md sticky top-0 z-20">
        <div className="container mx-auto max-w-2xl p-4">
          <h1 className="text-2xl font-bold text-center">TyronesMacros</h1>
        </div>
      </header>
      ...
      <Modal
        isOpen={isConfirmWorkoutDeleteOpen}
        onClose={() => setConfirmWorkoutDeleteOpen(false)}
      ...
      <Modal
        ...
        onClose={() => { setConfirmMealDeleteOpen(false); setMealPendingDelete(null); }}
      ...
      >
        ...
        <div className="flex gap-3 justify-center">
            <button
              type="button"
              onClick={() => { setConfirmMealDeleteOpen(false); setMealPendingDelete(null); }}
              className="bg-gray-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-600 transition"
            >
              Cancel
            </button>
...
// ---------- Today view (now shows Today’s Target + quick Edit) ----------
const TodayView: React.FC<{
  ...
  onEditTargets: () => void; // 🔴 NEW
  savedTargets: MacroSet;     // 🔴 NEW
}> = ({
  today, mealsToday, remaining, statuses,
  onAddMealClick, onWorkoutClick, onGenerateSuggestion, isGeneratingSuggestion,
  onDeleteWorkout, onRequestDeleteMeal, onEditTargets, savedTargets
}) => {
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-700 dark:text-gray-300">
          C: <strong>{savedTargets.calories}</strong> kcal &nbsp;|&nbsp; P: <strong>{savedTargets.protein}</strong> g &nbsp;|&nbsp; C: <strong>{savedTargets.carbs}</strong> g &nbsp;|&nbsp; F: <strong>{savedTargets.fat}</strong> g
        </p>
      </div>

      {/* Action buttons */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <button onClick={onAddMealClick} className="w-full bg-primary text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-purple-700 transition duration-300">Add Meal</button>
        <button onClick={onWorkoutClick} className="w-full bg-gray-600 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-gray-700 transition duration-300">Add/Edit Workout</button>
        <button onClick={onGenerateSuggestion} disabled={isGeneratingSuggestion} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-blue-700 transition duration-300 disabled:bg-blue-400">
...
const HistoryView: React.FC<{ days: Day[], meals: Meal[] }> = ({ days, meals }) => {
  ...
  return ( <div className="space-y-4"> <h2 className="text-2xl font-bold text-center mb-4">Daily History</h2> {sortedDays.map(day => <HistoryItem key={day.date} day={day} meals={meals.filter(m => m.date === day.date)} />)} </div> );
};
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
      <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div>
          <p className="font-bold text-lg">{new Date(day.date + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
          <div className="flex gap-2 text-xs">
            <span>C: <StatusIcon status={statuses.calories} /></span>
            <span>P: <StatusIcon status={statuses.protein} /></span>
            <span>C: <StatusIcon status={statuses.carbs} /></span>
            <span>F: <StatusIcon status={statuses.fat} /></span>
          </div>
        </div>
      ...
      {isOpen && (
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-4">
          <div> <h4 className="font-semibold">Meals Logged</h4> {meals.length > 0 ? meals.map(m => <p key={m.id} className="text-sm">{m.mealType}: {m.mealSummary}</p>) : <p className="text-sm">No meals.</p>} </div>
          <div> <h4 className="font-semibold">Workout</h4> <p className="text-sm whitespace-pre-wrap">{day.workoutLogged || "No workout logged."} ({Math.round(day.workoutKcal || 0)} kcal)</p> </div>
          <div> <h4 className="font-semibold">Swap Suggestions</h4> <p className="text-sm">{day.swapSuggestions || "No suggestions."}</p> </div>
        </div>
      )}
    </div>
  );
...
const TargetsAndProfileView: React.FC<{ 
  ...
  const handleProfileChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    setProfileState({ ...profileState, [e.target.name]: e.target.type === 'number' ? parseFloat(e.target.value) || null : e.target.value });
  };
  ...
  const handleTargetsSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onUpdateTargets(targetsState); // 🔴 this persists + updates today
  ...
  const applyTargetOption = (option: MacroSet) => {
    onUpdateTargets(option); // 🔴 persist + update today
    setTargetOptions(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleProfileSubmit} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="weight_lbs" value={profileState.weight_lbs || ''} onChange={handleProfileChange} placeholder="Required" required className="mt-1 block w-full input-style" />
          </div>
          <div>
            <label className="block text-sm font-medium">Height (in)</label>
            <input type="number" name="height_in" value={profileState.height_in || ''} onChange={handleProfileChange} className="mt-1 block w-full input-style" />
          </div>
          <div>
          ...
          <div>
            ...
            <select name="sex" value={profileState.sex} onChange={handleProfileChange} className="mt-1 block w-full input-style"><option value="">—</option><option>Male</option><option>Female</option></select>
          </div>
        </div>
        <button type="submit" className="w-full bg-primary text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-purple-700 transition duration-300">Save Profile</button>
      </form>
      ...
      <form onSubmit={handleTargetsSubmit} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-4">
        ...
        <div className="flex gap-2 items-end">
          <div className="flex-grow">
            ...
            <select value={goal} onChange={(e) => setGoal(e.target.value as Goal)} className="mt-1 block w-full input-style">
              <option value="maintain">Maintain</option><option value="cut_0_5">Cut (~0.5 lb/week)</option><option value="recomp">Recomp</option><option value="gain_0_25">Gain (~0.25 lb/week)</option>
            </select>
          </div>
          <button type="button" onClick={handleGenerateTargetOptions} disabled={isGeneratingOptions} className="bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow hover:bg-blue-700 transition duration-300 disabled:bg-blue-400">
            AI Options
          </button>
        </div>
        {targetOptions && (
          <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg space-y-3">
            ...
            {targetOptions.options.map(opt => (
              <div key={opt.label} className="text-center p-2 border rounded-md border-gray-300 dark:border-gray-600">
                ...
                <p className="text-sm">C:{opt.calories} P:{opt.protein} C:{opt.carbs} F:{opt.fat}</p>
                <button type="button" onClick={() => applyTargetOption(opt)} className="text-xs bg-green-600 text-white px-2 py-1 rounded mt-1">Use This</button>
              </div>
            ))}
          </div>
        )}
        ...
        <button type="submit" className="w-full bg-primary text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-purple-700 transition duration-300">Save Manual Targets</button>
      </form>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-4">
        ...
        <div className="flex gap-4">
          <button onClick={() => exportDaysToCSV(allDays)} className="flex-1 bg-green-600 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-green-700 transition duration-300">Export Days (CSV)</button>
          <button onClick={() => exportMealsToCSV(allMeals)} className="flex-1 bg-green-600 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-green-700 transition duration-300">Export Meals (CSV)</button>
        </div>
      </div>
    </div>
  );
...
const TargetInput: React.FC<{ label: string; name: keyof MacroSet; value: number; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; }> = ({ label, name, value, onChange }) => (
  <div> <label htmlFor={name} className="block text-sm font-medium text-gray-700 dark:text-gray-300">{label}</label> <input type="number" name={name} id={name} value={value} onChange={onChange} className="mt-1 block w-full input-style" /> </div>
);

// ---------- Add Meal Modal ----------
const AddMealModal: React.FC<{ isOpen: boolean; onClose: () => void; onSubmit: (meal: Omit<Meal, 'id'>) => void; date: string; }> = ({ isOpen, onClose, onSubmit, date }) => {
  ...
  const resetForm = useCallback(() => {
    setMealType('Snack'); setMealSummary(''); setMacros({ calories: 0, protein: 0, carbs: 0, fat: 0 }); setEstimationNote(null); setIsEstimating(false);
  }, []);

  const handleClose = () => { resetForm(); onClose(); };
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSubmit({ date, mealType, mealSummary, macros }); resetForm(); };
  const handleMacroChange = (e: React.ChangeEvent<HTMLInputElement>) => setMacros({ ...macros, [e.target.name]: parseInt(e.target.value) || 0 });

  const handleEstimateMacros = async () => {
    if (!mealSummary) { alert("Please enter a meal description first."); return; }
    setIsEstimating(true); setEstimationNote(null);
    try {
      const result = await estimateMacrosForMeal(mealSummary);
      setMacros(result.macros);
      setEstimationNote(result.note);
    } catch (error: any) { alert(error.message); } finally { setIsEstimating(false); }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Add Meal">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div> <label className="block text-sm font-medium">Meal Type</label> <select value={mealType} onChange={(e) => setMealType(e.target.value as any)} className="mt-1 block w-full input-style"> <option>Breakfast</option> <option>Lunch</option> <option>Dinner</option> <option>Snack</option> </select> </div>
        <div>
          <div className="flex justify-between items-center mb-1">
            <label className="block text-sm font-medium">Meal Summary</label>
            <button type="button" onClick={handleEstimateMacros} disabled={isEstimating} className="text-sm bg-blue-600 text-white font-semibold py-1 px-3 rounded-lg shadow-md hover:bg-blue-700 transition duration-200 disabled:bg-blue-400 disabled:cursor-not-allowed"> {isEstimating ? 'Estimating...' : 'Estimate with AI'} </button>
          </div>
          <input type="text" value={mealSummary} onChange={e => setMealSummary(e.target.value)} placeholder="e.g., 250g salmon, 1 cup risotto" required className="block w-full input-style" />
        </div>
        <div className="grid grid-cols-2 gap-4"> <TargetInput label="Calories" name="calories" value={macros.calories} onChange={handleMacroChange} /> <TargetInput label="Protein (g)" name="protein" value={macros.protein} onChange={handleMacroChange} /> <TargetInput label="Carbs (g)" name="carbs" value={macros.carbs} onChange={handleMacroChange} /> <TargetInput label="Fat (g)" name="fat" value={macros.fat} onChange={handleMacroChange} /> </div>
        {estimationNote && ( <div className="p-2 bg-gray-100 dark:bg-gray-700 rounded-md text-sm text-center text-gray-600 dark:text-gray-300"> <p><strong>AI Note:</strong> {estimationNote}</p> </div> )}
        <button type="submit" className="w-full bg-primary text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-purple-700 transition duration-300">Log Meal</button>
      </form>
    </Modal>
  );
};
...
const WorkoutModal: React.FC<{ 
  ...
  useEffect(() => { setWorkout(currentWorkout); setKcal(currentKcal); }, [currentWorkout, currentKcal, isOpen]);
  
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSubmit(workout, kcal); };
  ...
  const handleAiEstimate = async () => {
    ...
    if (!workout) { alert("Please enter a workout description first."); return; }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add/Edit Workout">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div> <label className="block text-sm font-medium">Workout Log</label> <textarea value={workout} onChange={e => setWorkout(e.target.value)} rows={6} className="mt-1 block w-full input-style"></textarea> </div>
        <div className="flex gap-2 items-center">
          <button type="button" onClick={handleAiEstimate} disabled={isEstimating} className="bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow hover:bg-blue-700 transition disabled:bg-blue-400">
            AI Workout Calories
          </button>
        ...
        <div> <h4 className="text-sm font-medium">Quick Add:</h4> <div className="flex flex-wrap gap-2 mt-2"> {WORKOUT_CHIPS.map(chip => ( <button key={chip} type="button" onClick={() => addChip(chip)} className="bg-gray-200 dark:bg-gray-600 text-sm px-3 py-1 rounded-full hover:bg-gray-300 dark:hover:bg-gray-500">{chip}</button> ))} </div> </div>
        <button type="submit" className="w-full bg-primary text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:bg-purple-700 transition duration-300">Save Workout</button>
      </form>
    </Modal>
  );
};
...
// ---------- Icons / Nav ----------
const ICONS = {
  home: <svg xmlns="http://www.w3.org/2000/svg" className="hero" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>,
  calendar: <svg xmlns="http://www.w3.org/2000/svg" className="hero" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2z" /></svg>,
  cog: <svg xmlns="http://www.w3.org/2000/svg" className="hero" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>,
};

const NavButton: React.FC<{ label: string; icon: keyof typeof ICONS; active: boolean; onClick: () => void; }> = ({ label, icon, active, onClick }) => (
  <button onClick={onClick} className={`flex-1 flex flex-col items-center justify-center py-2 px-1 text-center text-sm transition-colors duration-200 ${active ? 'text-primary' : 'text-gray-500 dark:text-gray-400 hover:text-primary'}`}>
    {ICONS[icon]}
